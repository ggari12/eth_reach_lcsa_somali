...
df_main_data <- readxl::read_excel("inputs/REACH_ETH_LCSA_Somali_data.xlsx") |> 
...
df_main_data_support_audit <- df_main_data |> 
  select(uuid = `_uuid`, enumerator_id, hh_kebele, pop_group, point_number, main_survey_time_interval)
...
saveWorkbook(wb, paste0("outputs/", butteR::date_file_prefix(),"_formatted_audit_summary_lcsa.xlsx"), overwrite = TRUE)
openXL(file = paste0("outputs/", butteR::date_file_prefix(),"_formatted_audit_summary_lcsa.xlsx"))
...
write_csv(x = df_extra_audit_log, file = paste0("outputs/", butteR::date_file_prefix(), 
                                                "_extra_audit_checks_lcsa.csv"), na = "")
