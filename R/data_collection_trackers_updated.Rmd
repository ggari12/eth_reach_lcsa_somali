---
title: "ETH LCSA Somali: Data Collection Tracker"
author: "REACH"
date: "Novomber 2023"
output:
    html_document:
...
toc_float:
    collapsed: false
smooth_scroll: false
knit: (function(inputFile, encoding) {rmarkdown::render(inputFile, encoding = encoding, output_file = paste0(dirname(inputFile), '/eth_lcsa_somali_data_collection_tracker_', format(Sys.Date(), '%Y_%m_%d'),'.html')) })
---
...
# read data
df_tool_data <- readxl::read_excel("../inputs/REACH_ETH_LCSA_Somali_data.xlsx")  |>  
  mutate(uuid = `_uuid`,
...
df_cl_log <- read_csv(file = "../inputs/combined_checks_eth_lcsa_somali.csv") |> 
...
# sample data
df_host_samples <- readxl::read_excel("../support_files/lcsa_sample_size.xlsx", sheet = "host") |> 
    select(hh_kebele, pop_group, required_samples = tot_sample_size)

df_idp_samples <- readxl::read_excel("../support_files/lcsa_sample_size.xlsx", sheet = "idp") |> 
    select(hh_kebele, pop_group, required_samples = tot_sample_size)
...
### Host community: **`r df_updated_tool_data %>% filter(pop_group == "host") %>% nrow()`** surveys

```{r, echo = FALSE}
df_samp_per_location <- df_host_samples
df_updated_tool_data |> 
  filter(pop_group == "host") |> 
  group_by(hh_kebele) |> 
  summarise(number_of_surveys = n()) |> 
  arrange(hh_kebele) |> 
  full_join(df_samp_per_location, by = c("hh_kebele" = "hh_kebele")) |>
  ...
  left_join(df_zone_to_kebele_info, by = c("hh_kebele" = "code")) |> 
  relocate(c(zone, woreda, hh_kebele, kebele), .before = number_of_surveys) |>
  dt_options_fewcols()
```

### IDPs community: **`r df_updated_tool_data %>% filter(pop_group == "idp") %>% nrow()`** surveys

```{r, echo = FALSE}
df_samp_per_location <- df_idp_samples
df_updated_tool_data |> 
...
```{r, echo = FALSE}
df_enum_performance |> 
group_by(hh_zone, hh_woreda, hh_kebele, start_date, enumerator_id) |>
  summarise(number_of_interviews_done = n(), `average_survey_time(minutes)` = round(mean(int.survey_time_interval, na.rm = TRUE), 0)) |> 
  left_join(df_zone_info, by = c("hh_zone" = "code")) |> 
  relocate(zone, .after = hh_zone) |> 
  left_join(df_woreda_info, by = c("hh_woreda" = "code")) |> 
  relocate(woreda, .after = hh_woreda) |>
  mutate(hh_kebele = as.character(hh_kebele)) |> 
  left_join(df_kebele_info, by = c("hh_kebele" = "code")) |> 
  relocate(kebele, .after = hh_kebele) |>
  ungroup() |> 
  select(-c(hh_zone, hh_woreda)) |> 
  dt_options_fewcols()
```
