...
# sample data
df_samples_required <- readxl::read_excel("../support_files/eth_lcsa_sample_size_somali.xlsx") |> 
    select(kebele, required_samples = tot_sample_size)
...
df_updated_tool_data |> 
    group_by(hh_kebele, pop_group) |> 
    summarise(number_of_surveys = n()) |> 
    arrange(pop_group) |> 
    full_join(df_samp_per_location, by = c("pop_group" = "pop_group")) |>
    mutate(required_samples = ifelse(is.na(required_samples), 0, required_samples)) |> 
    left_join(df_cl_surveys_for_deletion, by = "pop_group") |> 
...
df_enum_performance |> 
  ...
  ungroup() |> 
  select(-c(hh_zone, hh_woreda, hh_kebele)) |> 
  dt_options_fewcols()
```
